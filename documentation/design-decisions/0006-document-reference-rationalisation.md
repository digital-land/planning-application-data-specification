...
- `application.documents` remains the source of truth for metadata and files (reference, name, description, document-types, uploaded-date, file). `reference` is generated by the submitting system and must be unique/stable within the submission.  
