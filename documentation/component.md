...
Each field reference may appear only once directly within a component's `fields` list. Use distinct references for distinct roles, such as `capacity-unit` and `throughput-unit` in waste management. Reusing a field in separate nested objects is allowed; position in the list must not determine which property a value represents.
